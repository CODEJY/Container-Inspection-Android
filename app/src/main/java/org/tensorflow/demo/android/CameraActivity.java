...
public abstract class CameraActivity extends Activity
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    capture.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View view) {
        SingleIns.getInstance().isSend = true;
      }
    });
    mapValue.put('A',10);
    mapValue.put('B',12);
  ...
  public void receiveBitmEventBus(Bitmap bitmap_) {
    this.bitmap = bitmap_;
    new Thread(new Runnable() {
      @Override
      public void run() {
        //需不需要加一个循环等待bitmap不为null
        Bitmap bitmap_ = bitmapHandle.preProcess(bitmap);
        float[] data = bitmapHandle.getPixelData(bitmap_);
        recognizeImg.recognize(data);
        bitmap_.recycle();
      }
    }).start();
  ...
  public void receivedOutput(String output) {
    ...
    if (!checkOutput(output))
      output_edit.setError("箱号识别有误！");

  }
// 校验
  private boolean checkOutput(String s) {
    int num = 0;
    for (int i = 0; i < 10;i++) {
      if (i < 4 && Character.isUpperCase(s.charAt(i)))
        num += mapValue.get(s.charAt(i))* Math.pow(2,i);
      else if (i >= 4)
        num += Integer.parseInt(s.charAt(i)+"")* Math.pow(2,i);
      else
        return false;
    }
    if(num % 11 == Integer.parseInt(s.charAt(10)+""))
      return true;
    else
      return false;
  }
